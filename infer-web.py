...
def preprocess_dataset(trainset_dir, exp_dir, sr, n_p):
    ...
    f.close()
    cmd = '"%s" rvc_beta/infer/modules/train/preprocess.py "%s" %s %s "%s/logs/%s" %s %.1f' % (
        config.python_cmd,
        trainset_dir,
        sr,
        n_p,
...
def extract_f0_feature(gpus, n_p, f0method, if_f0, exp_dir, version19, gpus_rmvpe):
    ...
    if if_f0:
        if f0method != "rmvpe_gpu":
            cmd = (
                '"%s" rvc_beta/infer/modules/train/extract/extract_f0_print.py "%s/logs/%s" %s %s'
                % (
                    config.python_cmd,
                    now_dir,
                    exp_dir,
            ...
            if gpus_rmvpe != "-":
                ...
                for idx, n_g in enumerate(gpus_rmvpe):
                    cmd = (
                        '"%s" rvc_beta/infer/modules/train/extract/extract_f0_rmvpe.py %s %s %s "%s/logs/%s" %s '
                        % (
                            config.python_cmd,
                            leng,
                            idx,
                ...
                cmd = (
                    config.python_cmd
                    + ' rvc_beta/infer/modules/train/extract/extract_f0_rmvpe_dml.py "%s/logs/%s" '
                    % (
                        now_dir,
                        exp_dir,
                    )
    ...
    for idx, n_g in enumerate(gpus):
        cmd = (
            '"%s" rvc_beta/infer/modules/train/extract_feature_print.py %s %s %s %s "%s/logs/%s" %s %s'
            % (
                config.python_cmd,
                config.device,
                leng,
...
def click_train(
    ...
    if gpus16:
        cmd = (
            '"%s" rvc_beta/infer/modules/train/train.py -e "%s" -sr %s -f0 %s -bs %s -g %s -te %s -se %s %s %s -l %s -c %s -sw %s -v %s'
            % (
                config.python_cmd,
                exp_dir1,
                sr2,
